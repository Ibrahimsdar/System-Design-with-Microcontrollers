...
volatile uint16_t total=0;	// keep track of increment and total
...
ISR(TIMER0_OVF_vect){
	...
	incr = 0;
	sign = 10; 
	for(i=0;i<2;i++){	// loops to check on all the 2 buttons
		switch(chk_buttons(i)){	// switch statement to check if it's pressed or not
			...
			case 1: 	// if button is pressed
				switch(i){	// i is the index (which button was detected)
					case 0: fmode[i] = ~(fmode[i]); // PA0, increment by 2 mode
						break;
					case 1: fmode[i] = ~(fmode[i]); // PA1, increment by 4 mode
						break;
					default: break;
				} // inner switch
				break; // end of case 1
	...
	switch(sign){
		case 0: // decrement 
			...
			if (total > 1023){	// if out of bounderies, roll over starting from 1023
				dig_value[3] = 1;	// digit 1
				dig_value[2] = 0;	// digit 0
				dig_value[1] = 2;	// digit 2
				dig_value[0] = 3;	// digit 3
				for(i=1;i<4;i++){ // enable all digits
					dstate[i] = 1;
				}
				total=1023;	// set the total
			}
		...
		case 1:	// increment 
			...
			if (total > 1023){	// if out of bounderies, roll over starting from one
				...
				for(i=1;i<4;i++){
					dig_value[i] = 0;	// digit i
					dstate[i] = 0;	// display enable is cleared
				}
